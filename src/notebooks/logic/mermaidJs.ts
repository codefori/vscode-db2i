...
export type mermaidType = `bar` | `line` | `pie`;
export const chartJsTypes: mermaidType[] = [`bar`, `line`, `pie`];
...
export function generateChart(id: number, detail: ChartDetail, columns: string[], rows: any[]): string | undefined {
  if (rows.length === 1) {
    ...
    return generateMermaidChart(id, detail, labels, [{ data, label: `Data` }]);
    ...
    if (labelIndex >= 0) {
      ...
      return generateMermaidChart(id, detail, labels, datasets);
    }
  }
}

function generateMermaidChart(id: number, detail: ChartDetail, labels, datasets: Dataset[]): string {
  return ``;
}
