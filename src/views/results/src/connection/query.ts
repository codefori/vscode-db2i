...
export class Query<T> {
  ...
  //!!! NOTE FOR FUTURE ADD A OPTION FOR 'ROW'
  private transformQuery(query: string): string {
    ...
    query = query.replace(/^update/, 'SELECT COUNT(1) FROM');
    // Clear "SET"
    query = query.replace(/SET.*?WHERE/, 'WHERE');
    query = query.replace(/set.*?where/, 'where');
  ...
  public async run(rowsToFetch: number = this.rowsToFetch): Promise<QueryResult<T>> {
    ...
    if (this.isCLCommand) {
      ...
      if(this.sql.toUpperCase().includes("UPDATE")){
        queryObject1 = {
    ...
    if (queryObject1 !== null) {
      result1 = await this.job.send(JSON.stringify(queryObject1));
    }
    let result = await this.job.send(JSON.stringify(queryObject)); 
    let match: string = "0"; // Initialize match with a default value (an empty string).
        
    if (result1 !== null) {
      const regexMatch = /"00001":(\d+)}/.exec(result1);
      if (regexMatch) {
        match = regexMatch[1]; // Assign the entire matched substring to the match variable.
      }
    }

    const queryResult: QueryResult<T> = JSON.parse(result.slice(0, -2) + ',"rAffect":' + match + "}");
