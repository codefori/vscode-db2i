...
exports.getChildren = async (schema, name) => {
  ...
  return columns.map(column => new this.ParmTreeItem(schema, name, column));
}

exports.ParmTreeItem = class extends vscode.TreeItem {
  /**
   * @param {string} schema
   * @param {string} table
   * @param {SQLParm} data 
  ...
  constructor(schema, table, data) {
    super(data.PARAMETER_NAME, vscode.TreeItemCollapsibleState.None);
